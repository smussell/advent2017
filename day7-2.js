const input = `vpryah (310) -> iedlpkf, epeain
aqfemi (38)
...
wadeqe (98)
dqwocyn (1206) -> aepss, jtxphqg, gveca
unbjm (40)
qhwtf (97)
afxazvx (49)
pyraep (46)
...
const tokenizer = /([a-z]+) \(([0-9]+)\)( -> ([a-z,\s]+))?/
const tokenized = input.split('\n')
  .map(d => d.match(tokenizer))

const plist = tokenized.map(d => d[1])

const tree = tokenized.filter(d => d[4])
  .reduce((a, c) => ({[c[1]]: c[4].split(', '), ...a}), {})

const all = tokenized.filter(d => d[4])
  .reduce((a, c) => a + c[4] + ', ', '')

const weights = tokenized.reduce((a, c) => ({[c[1]]: parseInt(c[2], 10), ...a}), {})

const x = plist.filter(d => all.indexOf(d) === -1)
x

function compute(node) {
  if(tree[node]) {
    return weights[node] + tree[node].reduce((a, c) => a + compute(c), 0)
  }
  return weights[node];
}
...
function isBalanced(node) {
  if(tree[node]) {
    return allEq(tree[node].map(d => compute(d)))
  }
  return true;
}

// let q = plist.map(d => ({d, bal: isBalanced(d)}))
// let n = q.filter(d => !d.bal)
isBalanced('tylelk')
