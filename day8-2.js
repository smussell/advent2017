...
const input = `o inc 394 if tcy >= -3
s inc -204 if s <= -5
keq inc -34 if keq > -6
ip inc 762 if ip == 4
zr inc -644 if ip >= 6
...
input.forEach(d => {
  const [target, instr, amnt, _, source, op, val] = d;
  if(eval(`${regs[source] || 0} ${op} ${parseInt(val, 10)}`)) {
    let init = regs[target] || 0;
    regs[target] = instrs[instr](init, parseInt(amnt, 10));
    if(regs[target] > max) {
      max = regs[target];
    }
  }  
...
let v = Object.values(regs).sort((a, b) => b - a)
max
